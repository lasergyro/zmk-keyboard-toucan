...
class GlyphMixin:
    ...
    def legend_is_glyph(self, legend: str) -> str | None:
        """Return glyph name if the legend is exactly a glyph reference (no trailing text)."""
        if m := self._glyph_name_re.match(legend):
            name = m.group("glyph")
            if m.end() == len(legend) and name in self.name_to_svg:
                return name
        return None

    def legend_glyph_prefix(self, legend: str) -> tuple[str, str] | None:
        """If legend starts with a glyph ref followed by non-empty text (e.g. $$mdi:bluetooth$$1),
        return (glyph_name, suffix). Used to render icon+text side by side."""
        if m := self._glyph_name_re.match(legend):
            name = m.group("glyph")
            suffix = legend[m.end():]
            if suffix and name in self.name_to_svg:
                return name, suffix
        return None
