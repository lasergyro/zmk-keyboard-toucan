...
According to the [references/hut1_5.pdf](references/hut1_5.pdf), the Generic Desktop usage page (0x01) defines controls for the system, such as sleep, power, and do not disturb.
...
1. A new HID report ID `ZMK_HID_REPORT_ID_GENERIC_DESKTOP` needs to be defined in [app/include/zmk/hid.h](app/include/zmk/hid.h).
...
4. The key needs to be bound to the keyboard map ([toucan.dtsi](toucan.dtsi) or `toucan_right.keymap`).

For a reference implementation on adding generic desktop controls to ZMK, see [references/pr2473.patch](references/pr2473.patch).
